...
#include <string.h> //解决'strcat' was not declared in this scope
#include <assert.h>
#include <vector>
#include <iostream>
...
// ---- md5摘要哈希 ---- //
void CryptoUtils::md5(const string &srcStr, string &encodedStr, string &encodedHexStr)
{
    // 调用md5哈希
...
// ---- sha256摘要哈希 ---- //
void CryptoUtils::sha256(const std::string &srcStr, std::string &encodedStr, std::string &encodedHexStr)
{
    // 调用sha256哈希
    unsigned char mdStr[33] = {0};
...
// 私钥解密
std::string rsa_pri_decrypt(const string &cipherText, const string &priKey)
{
    string strRet;
    RSA *rsa = RSA_new();
    BIO *keybio;
...
int test()
{
    ...
    CryptoUtils::md5(srcText, encryptText, encryptHexText);
